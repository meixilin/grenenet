# Title: Run LMM regression on the scaled deltaP files
...
plotdir = paste0('plots/ver202209/LMM/', envvar, '/')
...
starttime = Sys.time()
bpparam <- SnowParam(workers=4, type="SOCK") # 4 or take more CPUs

# prepare the function you want to parallelize
FUN <- function(yy, metadt) {
    # yy = deltadt[ii,]
    # Account for experimental setup
    mydata = cbind(yy, metadt)
    lmemodel = suppressMessages(
...
lmres0 <- bplapply(deltadt, FUN, BPPARAM=bpparam)

lmres0 = foreach(i=1:nrow(deltadt), .packages = c('lmerTest', 'MuMIn')) %dopar% {
    # Account for experimental setup
    yy=deltadt[i, ]
    mydata = cbind(yy, metadt)
    lmemodel = suppressMessages(
